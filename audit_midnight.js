...
async function check() {
  ...
  console.log('\n--- Activity Logs around 12:05 AM ---');
  activityLogs.filter(e => new Date(e.createdAt).getHours() === 0 && new Date(e.createdAt).getMinutes() <= 10).forEach(e => {
    console.log(`[${new Date(e.createdAt).toLocaleString()}] ${e.userName}: ${e.text}`);
  });

  console.log('\n--- Executions around 12:05 AM ---');
  executedAutomations.filter(e => new Date(e.createdAt).getHours() === 0 && new Date(e.createdAt).getMinutes() <= 10).forEach(e => {
