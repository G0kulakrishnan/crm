...
export default function AllTasks({ user }) {
  ...
  const [form, setForm] = useState({ title: '', assignTo: '', dueDate: '', priority: 'Medium', status: 'Pending', notes: '', projectId: '' });
  ...
  return (
    <div>
      <div className="sh"><div><h2>All Tasks</h2></div><button className="btn btn-primary btn-sm" onClick={() => { setEditData(null); setForm({ title: '', assignTo: '', dueDate: '', priority: 'Medium', status: taskStatuses[0], notes: '', projectId: '' }); setModal(true); }}>+ Create Task</button></div>
      <div className="tabs">
      ...
      <div className="tw">
        ...
        <table>
          <thead><tr><th>#</th><th>Title</th><th>Project</th><th>Assigned To</th><th>Due Date</th><th>Priority</th><th>Status</th><th>Actions</th></tr></thead>
          <tbody>
            {filtered.length === 0 ? <tr><td colSpan={8} style={{ textAlign: 'center', padding: 28, color: 'var(--muted)' }}>No tasks</td></tr>
              : filtered.map((t, i) => (
                <tr key={t.id}>
                  ...
                  <td>
                    <button className="btn btn-secondary btn-sm" onClick={() => { setEditData(t); setForm({ title: t.title, assignTo: t.assignTo || '', dueDate: t.dueDate || '', priority: t.priority, status: t.status, notes: t.notes || '', projectId: t.projectId || '' }); setModal(true); }}>Edit</button>{' '}
                    <button className="btn btn-sm" style={{ background: '#fee2e2', color: '#991b1b' }} onClick={() => del(t.id)}>Del</button>
                  </td>
                </tr>
              ))}
          </tbody>
