...
// Known TradeIndia API response fields
const TRADEINDIA_COLUMNS = [
  'sender_name', 'sender_email', 'sender_mobile', 'sender_company',
  'sender_address', 'sender_city', 'sender_state', 'sender_country',
  'subject', 'query_message', 'product_name', 'inquiry_date',
  'inquiry_id', 'status'
];

export default function TradeindiaIntegration({ user, ownerId, onBack, existingConfig, editIndex }) {
  ...
  const columns = TRADEINDIA_COLUMNS;
  ...
  const webhookUrl = `${crmDomain}/api/webhook/tradeindia?userId=${ownerId}`;

  const handleSave = async () => {
    if (!configName) return toast('Please enter a name for this integration', 'error');
    if (!tiUserId) return toast('Please enter your TradeIndia User ID', 'error');
  ...
  const handleSync = async () => {
    ...
    setSyncResults(null);
    try {
      const res = await fetch(`${crmDomain}/api/webhook/tradeindia?userId=${ownerId}&action=sync`);
  ...
  const handleSendTestLead = async () => {
    try {
      // Build a test lead from sample TradeIndia data
      const sampleData = {
        sender_name: 'Test Lead (TradeIndia)',
        sender_email: 'test-tradeindia@example.com',
        sender_mobile: '9876543210',
        sender_company: 'Test Company',
        query_message: 'Test enquiry from TradeIndia integration',
        ...
        <div className="mapping-controls">
          ...
          {m.type === 'column' ? (
            <select value={m.value} onChange={e => updateVal({ value: e.target.value })}>
              <option value="">(Select TradeIndia Field)</option>
  ...
  return (
    <div className="sheet-config">
      <div className="sh" style={{ marginBottom: 20 }}>
        ...
        <div>
          <h3>{editIndex !== null ? 'Edit' : 'New'} TradeIndia Integration</h3>
          <div className="sub">Map TradeIndia lead fields to your CRM fields</div>
        </div>
      </div>
      ...
      <div className="tw" style={{ padding: 25, marginBottom: 20 }}>
        <div className="fg" style={{ marginBottom: 15 }}>
          ...
          <input
            ...
            placeholder="e.g. TradeIndia Leads 2024"
          ...
          <label>TradeIndia Profile ID</label>
          <input
            value={profileId}
            onChange={e => setProfileId(e.target.value)}
            placeholder="Enter your TradeIndia Profile ID..."
            style={{ width: '100%', marginBottom: 15 }}
          />

          <label>TradeIndia API Key</label>
          <input
            value={apiKey}
          ...
          <div style={{ fontSize: 10, color: 'var(--muted)', marginBottom: 15 }}>
            Get these from your TradeIndia Dashboard → Inquiries & Contacts → My Inquiry API
          </div>
          ...
          <div style={{ fontSize: 10, color: 'var(--muted)', marginBottom: 15 }}>
            Register this URL in your TradeIndia dashboard to receive leads automatically in real-time.
          </div>
        ...
        <div className="gs-success-msg">
          ...
          <div>
            <strong>TradeIndia — {columns.length} fields available for mapping</strong>
            <div style={{ fontSize: 11 }}>Map each CRM field below to a TradeIndia field or set a fixed value.</div>
          </div>
        </div>
      </div>
